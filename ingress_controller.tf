provider "kubernetes" {
  ...
  config_path = "/Users/anbhalla/.kube/demo-vn-config"
}
...
resource "null_resource" "create_ingress_controller" {
  ...
  provisioner "local-exec" {
    command = "kubectl --kubeconfig /Users/anbhalla/.kube/demo-vn-config create -f https://raw.githubusercontent.com/oracle-devrel/oci-oke-virtual-nodes/main/ingress-nginx/deploy.yaml"
  }
}
...
resource "null_resource" "annotate_ingress_controller" {
  ...
  provisioner "local-exec" {
    command = "kubectl --kubeconfig /Users/anbhalla/.kube/demo-vn-config  -n ingress-nginx annotate svc ingress-nginx-controller oci.oraclecloud.com/oci-network-security-groups=${oci_core_network_security_group.ingress_controller.id}"
  }
}